...
from middleware.modules.plans_mgmt.persistence.entities.draft_plan import DraftPlan
from middleware.modules.plans_mgmt.persistence.interfaces import DraftPlanDao
...
def _to_dict(row: DraftPlan) -> dict:
    return {
        "id": row.id,
...
class DraftPlanDaoImpl(DraftPlanDao):
    @override
    def upsert(self, record: dict) -> dict:
        now = datetime.now(timezone.utc)
        with session_scope() as session:
            row = (
                session.query(DraftPlan)
                .filter(DraftPlan.thread_id == record["thread_id"])
                .one_or_none()
            )
            if row is None:
                row = DraftPlan(
    ...
    def get_for_user(self, user_id: str, thread_id: str) -> dict | None:
        with session_scope() as session:
            row = (
                session.query(DraftPlan)
                .filter(
                    DraftPlan.user_id == user_id,
                    DraftPlan.thread_id == thread_id,
                )
                .one_or_none()
            )
    ...
    def delete_for_user(self, user_id: str) -> None:
        with session_scope() as session:
            session.query(DraftPlan).filter(DraftPlan.user_id == user_id).delete()
